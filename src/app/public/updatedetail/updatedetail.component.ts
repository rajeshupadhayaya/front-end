...
export class UpdatedetailComponent implements OnInit {
  ...
  emailverify() {
    this.userService.generateVerifyEmail().subscribe(
      (data: any) => {
        var msg =
          "We have sent you a e-mail with verification link, Please check the email";
        this.success(msg);
      },
      (err: HttpErrorResponse) => {
        var msg =
          "We are unable to verify Email currently, please try after some time";
        this.error(msg);
      }
    );
  }
